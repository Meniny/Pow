...
class PowContentView: UIView {
    ...
    // Entry delegate
    private weak var powDelegate: EntryContentViewDelegate!
    ...
    // Data source
    private var attributes: PowAttributes!
    ...
    init(withEntryDelegate powDelegate: EntryContentViewDelegate) {
        self.powDelegate = powDelegate
        super.init(frame: .zero)
    }
    
    // Called from outer scope with a presentable view and attributes
    func setup(with contentView: UIView, attributes: PowAttributes) {
    ...
    private func setupInitialPosition() {
        ...
        // Determine the layout entrance type according to the pow type
        ...
        let safeAreaInsets = PowWindowProvider.safeAreaInsets
        ...
        let setupOutConstraint = { (animation: PowAttributes.Animation, priority: UILayoutPriority) -> NSLayoutConstraint in
    ...
    // Setup layout constraints according to PowAttributes.PositionConstraints
    ...
    private func setupTapGestureRecognizer() {
        guard attributes.powInteraction.isResponsive else {
            return
        }
    ...
    private func generateHapticFeedback() {
        ...
        PowHapticFeedbackGenerator.notification(type: attributes.hapticFeedbackType)
    }
    ...
    func animateOut(pushOut: Bool) {
        ...
        outDispatchWorkItem?.cancel()
        powDelegate?.changeToInactive(withAttributes: attributes)
    ...
    // Animate out
    private func animateOut(with animation: PowAttributes.Animation, outTranslationType: OutTranslation) {
    ...
    private func animateIn() {
        ...
        powDelegate?.changeToActive(withAttributes: attributes)
    ...
    private func performAnimation(with animation: PowAnimation, preAction: @escaping () -> () = {}, action: @escaping () -> ()) {
    ...
    func removeFromSuperview(keepWindow: Bool) {
        ...
        if PowAttributes.count > 0 {
            PowAttributes.count -= 1
        }
        if !keepWindow && !PowAttributes.isDisplaying {
            PowWindowProvider.shared.state = .main
        }
    }
...
extension PowContentView {
    ...
    private func animate(by userInfo: [AnyHashable: Any]?, entrance: Bool) {
        ...
        // Guard that the pow is bound to the keyboard
...
extension PowContentView {
    ...
    @objc func tapGestureRecognized() {
        switch attributes.powInteraction.defaultAction {
        ...
        attributes.powInteraction.customTapActions.forEach { $0() }
    }
    ...
    @objc func panGestureRecognized(gr: UIPanGestureRecognizer) {
        ...
        // Delay the exit of the pow if needed
    ...
    private func animateRubberBandPullback() {
        ...
        let animation: PowAttributes.Scroll.PullbackAnimation
    ...
    private func handleExitDelayIfNeeded(byPanState state: UIGestureRecognizerState) {
        guard attributes.powInteraction.isDelayExit && attributes.displayDuration.isFinite else {
            return
        }
        switch state {
        case .began:
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if attributes.powInteraction.isDelayExit && attributes.displayDuration.isFinite {
            outDispatchWorkItem?.cancel()
        }
    }
