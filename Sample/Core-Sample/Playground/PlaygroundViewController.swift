...
class PlaygroundViewController: UIViewController {
    ...
    private lazy var attributesWrapper: PowAttributeWrapper = {
        ...
        return PowAttributeWrapper(with: attributes)
    }()
    ...
    @IBAction func play() {
        let title = PowProperty.LabelContent(text: "Hi there!", style: PowProperty.LabelStyle(font: MainFont.bold.with(size: 16), color: .black))
        let description = PowProperty.LabelContent(text: "Are you ready for some testing?", style: PowProperty.LabelStyle(font: MainFont.light.with(size: 14), color: .black))
        let image = PowProperty.ImageContent(image: UIImage(named: "ic_info_outline")!, size: CGSize(width: 30, height: 30))
        ...
        Pow.display(contentView, using: attributesWrapper.attributes)
    }
}
...
extension PlaygroundViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case (1, 4):
            let cell = cell as! BackgroundStyleSelectionTableViewCell
            cell.configure(attributesWrapper: attributesWrapper, focus: .pow)
        case (2, 0):
